...
| Kind | Path |
| --- | --- |
| **Localized (UI chrome)** | `apps/web/public/docs/{en\|pl\|th}/` |
| **Shared (language-neutral)** | `apps/web/public/docs/shared/` |

Capture UI in the matching language for localized shots. If `pl`/`th` is
...
Replace `{locale}` with `en`, `pl`, or `th`.

### Capture status

Captured against `https://chemfox.ase.cx` into `apps/web/public/docs/{en|pl|th}/`
by switching the navbar language control (and `NEXT_LOCALE`).

| Filename | en | pl | th | Notes |
| --- | --- | --- | --- | --- |
| `client-my-experiments-board.webp` | Done | Done | Done | |
| `client-request-catalog.webp` | Done | Done | Done | |
| `client-request-intake.webp` | Done | Done | Done | |
| `client-experiment-detail.webp` | Done | Done | Done | |
| `client-sample-label.webp` | Done | Done | Done | Same REQUESTED detail (includes QR label) |
| `client-notification-settings.webp` | Done | Done | Done | |
| `client-report-panel.webp` | Done | Done | Done | client01 · `159c7177-…` |
| `staff-experiments-listing.webp` | Done | Done | Done | |
| `staff-sample-checkin.webp` | Done | Done | Done | |
| `staff-workspace-start.webp` | Done | Done | Done | |
| `staff-lab-form-collab.webp` | Done | Done | Done | |
| `staff-workspace-tabs.webp` | Done | Done | Done | |
| `staff-finalize-panel.webp` | Done | Done | Done | |
| `staff-onboarding-samples.webp` | Done | Done | Done | |
| `staff-sample-templates.webp` | Done | Done | Done | |
| `staff-template-builder.webp` | Done | Done | Done | |
...
| `staff-component-reference.webp` | Done | Done | Done | Prod `/internal/docs/string` |
...
# Polish + Thai (navbar language switch)
DOCS_LOCALES=pl,th \
DOCS_BASE_URL=https://chemfox.ase.cx \
DOCS_CLIENT_EMAIL=… DOCS_CLIENT_PASSWORD=… \
DOCS_STAFF_EMAIL=… DOCS_STAFF_PASSWORD=… \
...
Then add `pl/` and `th/` copies when you translate the UI screenshots.
